function [x,L,LAMBDA]=l2lqFract(A,b,options)
...
% options.LG: graph Laplacian (if not defined or empty the graph Laplacian
%             is computed using TIkhonov in general form with options.L as 
%             regularization operator and regularization parameter defined 
%             with the GCV);
...
% Check input
if ~isfield(options,'LG') || isempty(options.LG)
    flagLG=1;
else
    flagLG=0;
end
...
if ~isfield(options,'waitbar') || isempty(options.waitbar)
    ww=0;
else
    ww=options.waitbar;
end
...
% Verbose
fprintf('------ L2-Lq Results -------\n');
fprintf('Iterations:        %i       \n', k);
fprintf('Stopping:          %d       \n', norm(y-[y_old;0],'fro')/norm([y_old;0],'fro'));
fprintf('mu:                %d       \n', eta/c);
fprintf('----------------------------\n');
if ww~=0
    try
